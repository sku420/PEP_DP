...
void dijkastra(list<pair<int, int> >*adj,int u,int n)
{
	vector<bool>visited(n,0);
	vector<int>weight(n,10000000);
	...
	while(!q.empty())
	{
	    ...
	    for(auto it=adj[u].begin();it!=adj[u].end();it++)
            if (!visited[(*it).first] and weight[u]+(*it).second<weight[(*it).first])
            {
                weight[(*it).first]=weight[u]+(*it).second;
